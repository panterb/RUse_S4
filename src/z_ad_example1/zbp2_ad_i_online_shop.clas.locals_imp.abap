...
CLASS lsc_zhb_i_online_shop_act IMPLEMENTATION.
  ...
  METHOD save_modified.
    DATA : lt_online_shop_as TYPE STANDARD TABLE OF zad_onshop_ad,
           ls_online_shop_as TYPE                   zad_onshop_ad.
    IF zbp2_ad_i_online_shop=>cv_pr_mapped-purchaserequisition IS NOT INITIAL.
      LOOP AT zbp2_ad_i_online_shop=>cv_pr_mapped-purchaserequisition ASSIGNING FIELD-SYMBOL(<fs_pr_mapped>).
    ...
    IF create-online_shop IS NOT INITIAL.
      ...
      insert zad_onshop_ad FROM TABLE @lt_online_shop_as.
    ENDIF.
  ENDMETHOD.
   ...
   WITH VALUE #(
                 (    %cid_ref = 'My%CID_1'
                      %target = VALUE #(
                                       (  %cid                            = 'My%ItemCID_1'
                                          plant                           = '1010'
                                          purchaserequisitionitemtext     = 'PAAS API Test 19.05'
...
CREATE BY \_purchasereqnitemtext
   ...
   WITH VALUE #(  (   %cid_ref = 'My%ItemCID_1'
                      %target  = VALUE #( (
                                          textobjecttype = 'B01'
                                          language       = 'E'
                                          plainlongtext  = 'item text created from PAAS API 19.05'
                                        ) (
                                          textobjecttype = 'B02'
                                          language       = 'E'
      ...
      zbp2_ad_i_online_shop=>cv_pr_mapped = ls_pr_mapped.
